...
def upload_excellent(jsonData):
    ...
    for res in jsonData['data']:
        ...
        Excellentresult.update_excellent_result_by_all(res['s_id'],res['vl_id'],res['excellent_result'])

def upload_graduate(jsonData):
    # 上传到学生毕业和是否授予学位的表
    for res in jsonData['data']:
        # if int(res['graduate_result']) != 0 or int(res['degree_result']) != 0:
        Graduateresult.update_graduatew_result_by_all(res['s_id'],res['vl_id'],res['graduate_result'],res['degree_result'])